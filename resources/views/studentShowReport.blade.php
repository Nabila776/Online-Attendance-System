...
@section('content')
    <div class="relative min-h-screen flex">
        ...
        <div class="flex-1 p-10 text-2xl font-bold">
            ...
            <p class="text-center py-2">Students report Page</p>
            <div class="flex justify-center py-3">
            <a href="{{route('convertPdf',$student->id)}}" class="btn btn-danger">Convert into PDF</a>
            </div>
            <div class="w-full px-0 md:px-6 py-12">
                <h3 class="text-gray-700 uppercase font-bold mb-2">Attendance List</h3>
                <div class="flex items-center bg-gray-200 rounded-tl rounded-tr">
                    <div class="w-1/3 text-left text-gray-600 py-2 px-4 font-semibold">Student Id</div>
                    <div class="w-1/3 text-right text-gray-600 py-2 px-4 font-semibold">Name</div>
                    <div class="w-1/3 text-right text-gray-600 py-2 px-4 font-semibold">Attendance Date</div>
                </div>

                @foreach ($student->attendances as $report)
                    <div class="w-full flex items-center justify-between border border-gray-200">
                        <div class="text-left text-gray-600 py-2 px-4 font-medium ">{{ $report->student_id}}</div>
                        <div class="text-left text-gray-600 py-2 px-4 font-medium">{{ $student->first_name }}_{{ $student->last_name }}</div>
                        <div class="text-left text-gray-600 py-2 px-4 font-medium">{{ $report->attendance_date }}</div>
                    </div>
                @endforeach
            </div>
